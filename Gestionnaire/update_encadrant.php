<?php
// update_encadrant.php
// Allows ONLY Gestionnaire users to update an Encadrant user in the Users table, protected by JWT.
...
// --- Process Request to Update Encadrant (only if authenticated and authorized) ---
if ($_SERVER["REQUEST_METHOD"] == "PUT") {
    // Get userID from URL query parameter (e.g., /update_encadrant.php?userID=123)
    $userID = $_GET['userID'] ?? null;

    // Get update data from raw JSON body
    ...
    if (isset($input['email'])) {
        ...
        $sql_check_email = "SELECT userID FROM Users WHERE email = ? AND userID != ?";
    ...
    if (isset($input['role']) && $input['role'] !== 'Encadrant') {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Role cannot be changed from Encadrant using this endpoint.']);
        $mysqli->close();
        exit();
    }
    ...
    // Crucially, ensure we only update users with the 'Encadrant' role.
    $sql = "UPDATE Users SET $setClause WHERE userID = ? AND role = 'Encadrant'";
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        try {
            if ($stmt->execute()) {
                if ($stmt->affected_rows > 0) {
                    ...
                    $check_exist_sql = "SELECT userID FROM Users WHERE userID = ? AND role = 'Encadrant'";
                    if ($check_exist_stmt = $mysqli->prepare($check_exist_sql)) {
                        $check_exist_stmt->bind_param("i", $userID); // This bind_param is fine as it's not dynamic
                        $check_exist_stmt->execute();
                        $check_exist_stmt->store_result();
                        if ($check_exist_stmt->num_rows == 0) {
                             http_response_code(404);
                             $response['status'] = 'error';
                             $response['message'] = 'Encadrant not found or specified user is not an Encadrant.';
                        } else {
                             $response['status'] = 'info'; // Changed from success to info as no actual change happened
                             $response['message'] = 'Encadrant data is already up-to-date (no changes made).';
                        }
                        $check_exist_stmt->close();
                    } else {
                        http_response_code(500);
                        $response['status'] = 'error';
                        $response['message'] = 'Database error checking encadrant existence: ' . $mysqli->error;
                    }
                }
            } else {
                http_response_code(500);
        ...
        http_response_code(500);
        $response['status'] = 'error';
        $response['message'] = 'Error preparing SQL statement for encadrant update: ' . $mysqli->error;
    }
} else {
